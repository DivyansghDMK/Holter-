...
class ECGTestPage(QWidget):
    ...
    def __init__(self, test_name, stacked_widget):
        ...
        # ── Holter recording subsystem ─────────────────────────────────────────
        self._holter = None          # HolterStreamWriter (set when recording starts)
        self._holter_worker = None   # HolterAnalysisWorker
        self._holter_status_bar = None  # HolterStatusBar widget
        self._holter_target_hours = 24
            ...
            if is_packet_reader:
                ...
                try:
                    ...
                    for packet in packets:
                        ...
                        # ── Holter: push packet to disk writer ────────────────
                        if self._holter is not None and self._holter.is_running:
                            try:
                                self._holter.push(packet)
                            except Exception:
                                pass
                        # ──────────────────────────────────────────────────────
                        try:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # HOLTER MONITOR METHODS
    # ══════════════════════════════════════════════════════════════════════════

    def show_holter_menu(self):
        """Called when 'Holter' menu button is clicked. Shows sub-menu dialog."""
        from PyQt5.QtWidgets import QDialog, QVBoxLayout, QPushButton, QLabel
        dialog = QDialog(self)
        dialog.setWindowTitle("Holter Monitor")
        ...
        title = QLabel("🫀  Holter Monitor")
        title.setStyleSheet("color: #E65100; font-size: 16px; font-weight: bold; padding: 8px;")
        layout.addWidget(title)

        def _btn(text, color, handler):
            b = QPushButton(text)
            ...
            """)
            b.clicked.connect(lambda: (dialog.accept(), handler()))
            layout.addWidget(b)

        # Show appropriate button based on current state
        if self._holter is not None and self._holter.is_running:
            _btn("⬛  Stop Holter Recording", "#B71C1C", self._holter_stop)
        else:
            _btn("▶  Start New Holter Recording", "#2E7D32", self._holter_start)
            _btn("📂  Open & Analyze Saved Recording", "#1565C0", self._holter_open)

        cancel = QPushButton("Cancel")
        cancel.setStyleSheet("background: #333; color: white; border: none; border-radius: 6px; padding: 8px; font-size: 12px;")
        cancel.clicked.connect(dialog.reject)
        layout.addWidget(cancel)
        dialog.exec_()

    def _holter_start(self):
        """Show start dialog and begin Holter recording."""
        from ecg.holter.holter_ui import HolterStartDialog, HolterStatusBar
        from ecg.holter.stream_writer import HolterStreamWriter
        from ecg.holter.analysis_worker import HolterAnalysisWorker

        # Get patient info from existing settings if available
        existing_info = {}
        try:
            existing_info = self.settings_manager.get_setting("patient_details", {}) or {}
        except Exception:
            pass

        # Output directory: recordings/ next to project root
        output_dir = os.path.join(os.path.dirname(os.path.dirname(
            os.path.abspath(__file__))), "recordings")
        os.makedirs(output_dir, exist_ok=True)

        dialog = HolterStartDialog(self, patient_info=existing_info, output_dir=output_dir)
        ...
        patient_info, duration_hours, out_dir = result
        self._holter_target_hours = duration_hours

        # Create stream writer
        self._holter = HolterStreamWriter(
            output_dir=out_dir,
            patient_info=patient_info,
            fs=500,
            on_arrhythmia=self._holter_on_arrhythmia,
        )
        session_dir = self._holter.start()

        # Start analysis worker
        self._holter_worker = HolterAnalysisWorker(
            analysis_queue=self._holter.analysis_queue,
            on_chunk_done=self._holter_on_chunk_done,
            on_arrhythmia=lambda a, t: self._holter_on_arrhythmia(a, t),
            fs=500,
        )
        self._holter_worker.start()

        # Add status bar above the ECG grid
        self._holter_status_bar = HolterStatusBar(self, target_hours=duration_hours)
        self._holter_status_bar.stop_requested.connect(self._holter_stop)

        # Insert status bar into the main layout (above grid)
        main_layout = self.grid_widget.layout()
        if main_layout is not None:
            # Wrap in a vertical container
            from PyQt5.QtWidgets import QVBoxLayout, QWidget
            # Try to add to the parent layout
            parent_layout = self.layout()
            if isinstance(parent_layout, __import__('PyQt5.QtWidgets', fromlist=['QStackedLayout']).QStackedLayout):
                # Can't easily add to stacked layout — use overlay
                self._holter_status_bar.setParent(self.grid_widget)
                self._holter_status_bar.raise_()
                self._holter_status_bar.setGeometry(0, 0, self.grid_widget.width(), 48)
                self._holter_status_bar.show()
        self._holter_status_bar.show()

        # Status update timer
        self._holter_update_timer = __import__('PyQt5.QtCore', fromlist=['QTimer']).QTimer(self)
        self._holter_update_timer.timeout.connect(self._holter_update_status)
        self._holter_update_timer.start(3000)  # update every 3 seconds

        from PyQt5.QtWidgets import QMessageBox
        QMessageBox.information(self, "Holter Started",
            f"✅ Holter recording started!\n\n"
            f"Duration: {duration_hours} hours\n"
            f"Saving to: {session_dir}\n\n"
            f"Recording indicator shown on screen.\n"
            f"All ECG data is being saved to disk automatically.")

    def _holter_stop(self):
        """Stop the running Holter recording and open analysis window."""
        if self._holter is None:
            return

        from PyQt5.QtWidgets import QMessageBox, QProgressDialog, QApplication

        reply = QMessageBox.question(self, "Stop Holter Recording",
            "Stop Holter recording and generate analysis?",
            QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply != QMessageBox.Yes:
            return

        # Stop update timer
        if hasattr(self, '_holter_update_timer') and self._holter_update_timer:
            self._holter_update_timer.stop()

        # Remove status bar
        if self._holter_status_bar:
            self._holter_status_bar.cleanup()
            self._holter_status_bar.hide()
            self._holter_status_bar.deleteLater()
            self._holter_status_bar = None

        # Finalize recording
        progress = QProgressDialog("Finalizing Holter recording...", None, 0, 0, self)
        progress.setWindowModality(__import__('PyQt5.QtCore', fromlist=['Qt']).Qt.WindowModal)
        progress.show()
        QApplication.processEvents()

        summary = self._holter.stop()
        session_dir = summary.get('session_dir', '')
        patient_info = self._holter.patient_info

        # Wait for analysis worker to finish
        if self._holter_worker:
            self._holter_worker.join(timeout=30)
        ...
        if session_dir and os.path.exists(session_dir):
            self._holter_open_session(session_dir, patient_info)

    def _holter_open(self):
        """Open file dialog to load a saved .ecgh recording."""
        from PyQt5.QtWidgets import QFileDialog
        recordings_dir = os.path.join(os.path.dirname(os.path.dirname(
            os.path.abspath(__file__))), "recordings")
        session_dir = QFileDialog.getExistingDirectory(
            self, "Select Holter Session Folder", recordings_dir)
        if session_dir:
            self._holter_open_session(session_dir, {})

    def _holter_open_session(self, session_dir: str, patient_info: dict):
        """Open the Holter analysis window for a session."""
        from ecg.holter.holter_ui import HolterMainWindow
        win = HolterMainWindow(self, session_dir=session_dir, patient_info=patient_info)
        win.setWindowModality(__import__('PyQt5.QtCore', fromlist=['Qt']).Qt.NonModal)
        win.show()
        win.raise_()
        self._holter_window = win   # keep reference

    def _holter_update_status(self):
        """Called every 3 seconds to refresh status bar during recording."""
        if self._holter and self._holter.is_running and self._holter_status_bar:
            stats = self._holter.get_live_stats()
            self._holter_status_bar.update_stats(
                bpm=stats.get('bpm', 0),
                arrhythmias=stats.get('arrhythmias', [])[:3]
            )
            # Reposition status bar if grid was resized
            if self._holter_status_bar.parent() == self.grid_widget:
                self._holter_status_bar.setGeometry(
                    0, 0, self.grid_widget.width(), 48)

    def _holter_on_chunk_done(self, result: dict):
        """Called by analysis worker after each 30s chunk is analyzed."""
        if self._holter and result.get('hr_mean', 0) > 0:
            self._holter.update_live_stats(
                bpm=result['hr_mean'],
                arrhythmias=result.get('arrhythmias', [])
            )

    def _holter_on_arrhythmia(self, arrhythmia, timestamp):
        """Called when an arrhythmia is detected — update status bar ticker."""
        if self._holter_status_bar:
            labels = arrhythmia if isinstance(arrhythmia, list) else [arrhythmia]
            self._holter_status_bar.update_stats(bpm=0, arrhythmias=labels)
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            if hasattr(self, '_holter') and self._holter and self._holter.is_running:
                try:
                    self._holter.stop()
                except Exception:
                    pass
            if hasattr(self, '_holter_worker') and self._holter_worker:
                try:
                    self._holter_worker.join(timeout=5)
                except Exception:
                    pass
