...
class HolterOverviewPanel(QWidget):
    ...
    def update_summary(self, summary: dict):
        rows = [
            ...
            ("X Total (unclassified)", f"{summary.get('x_total', 0)} (<0.10%)"),
            ...
            ("During Tachy.",       f"{summary.get('tachy_dur', '00:00:00')}"),
            ("During Brady.",       f"{summary.get('brady_dur', '00:00:00')}"),
            ("V Total",             f"{summary.get('v_total', 0)} (<0.10%)"),
            ("S Total",             f"{summary.get('s_total', 0)} (<0.10%)"),
            ("AVG HR of Af/AF",     f"{summary.get('af_avg_hr', '—')}"),
            ("Duration of Af/AF",   f"{summary.get('af_duration', '00:00:00 <0.10%')}"),
            ("Paced Beats",         f"{summary.get('paced_beats', 0)} (<0.10%)"),
            ("Longest RR Interval", f"{summary.get('longest_rr_ms', 0):.2f}s"),
            ("RRI (≥2.0s)",         f"{summary.get('pauses', 0)}"),
            ("ST Elevation",        f"{summary.get('st_elevation', '—')}"),
            ("ST Depression",       f"{summary.get('st_depression', '—')}"),
        ]
...
class HolterMainWindow(QDialog):
    """
    Full Holter analysis window opened after a recording is complete.
    Tabs: Overview | HRV | Events | Replay | Report
    """

    def __init__(self, parent=None, session_dir: str = "",
                 patient_info: dict = None):
        super().__init__(parent)
        self.setWindowTitle("Holter ECG Analysis")
